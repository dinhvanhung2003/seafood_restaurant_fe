// app/(admin)/tables/page.tsx (hoặc tương tự)
"use client";
...
const toApiStatus = (s: ModalStatus): ApiStatus => (s === "active" ? "ACTIVE" : "INACTIVE");
const toModalStatus = (s: ApiStatus): ModalStatus => (s === "ACTIVE" ? "active" : "inactive");
...
export default function TablesPage() {
  ...
  // tables (server paging) – CHUYỂN items -> data
  ...
  const onOpenEditTable = (t: DiningTableDTO) => {
    ...
    setForm({
      ...
      // BE trả sortOrder → map về field order của form
      order: t.sortOrder ?? 0,
    });
    setOpenTableModal(true);
  };
  ...
  const handleSubmitTable = () => {
    ...
    if (tableEditing && form.id) {
      updateTable.mutate(
        ...
        {
          onSuccess: () => {
            toast.success("Đã cập nhật bàn", form.name);
      ...
      createTable.mutate(payload, {
        onSuccess: () => {
          toast.success("Đã tạo bàn", form.name);
          setOpenTableModal(false);
        },
    ...
    <div className="space-y-4">
            ...
            {error ? (
              <TableRow>
                <TableCell colSpan={5} className="py-10 text-center text-red-600">
                  {String((error as any)?.message || "Có lỗi xảy ra")}
                </TableCell>
              </TableRow>
            ) : tables.length === 0 ? (
              <TableRow>
                  ...
                  <TableCell>
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs ${
                        t.status === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-700"
                      }`}
                    >
                      {t.status}
                    </span>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Tổng {meta.total} bàn · Trang {meta.page}/{meta.pages || 1}
        </div>
        <div className="flex gap-2">
          <Button
            variant="outline"
