...
import MapPicker from "./GeoPicker";
      ...
      <Card className="p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <MapPicker value={place} onChange={setPlace} height={340} className="rounded overflow-hidden" />
            ...
            <Button
              onClick={() => {
                ...
                if (!name.trim()) return;
                if (!Number.isFinite(lat) || lat < -90 || lat > 90) return alert("Lat phải [-90, 90]");
                if (!Number.isFinite(lng) || lng < -180 || lng > 180) return alert("Lng phải [-180, 180]");
                if (!Number.isFinite(radius) || radius < 1) return alert("Radius >= 1");
            ...
            {geo.data?.length ? (
              geo.data.map((g) => (
                <div key={g.id} className="flex items-center justify-between border rounded p-2">
                  ...
                  <Button variant="destructive" size="sm" onClick={() => delGeo.mutate(g.id)}>
                    Xoá
                  </Button>
                </div>
              ))
            ...
            {net.data?.length ? (
              net.data.map((nr) => (
                <div key={nr.id} className="flex items-center justify-between border rounded p-2">
                  ...
                  <Button variant="destructive" size="sm" onClick={() => delNet.mutate(nr.id)}>
                    Xoá
                  </Button>
                </div>
              ))
