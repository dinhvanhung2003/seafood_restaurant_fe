"use client";

import { Dispatch, SetStateAction } from "react";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  Select, SelectTrigger, SelectValue, SelectContent, SelectItem,
} from "@/components/ui/select";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import { Plus } from "lucide-react";

type Status = "active" | "inactive";
export type Category = { id: string; name: string };

type Props = {
  categoryId: string;
  setCategoryId: Dispatch<SetStateAction<string>>;
  status: Status | "all";
  setStatus: Dispatch<SetStateAction<Status | "all">>;
  q: string;
  setQ: Dispatch<SetStateAction<string>>;
  categories: Category[];
  total: number;
  setShowAddCategory: Dispatch<SetStateAction<boolean>>;
};

export default function ProductFilters({
  categoryId, setCategoryId,
  status, setStatus,
  q, setQ,
  categories, total,
  setShowAddCategory,
}: Props) {
  return (
    <div className="flex flex-col space-y-4">
      <div className="rounded-lg border p-4 bg-white shadow-sm">
        <Label className="mb-1 block flex items-center justify-between">
          Danh mục
          <button
            type="button"
            onClick={() => setShowAddCategory(true)}
            className="text-sm text-blue-600 hover:underline ml-2"
            title="Thêm danh mục"
          >
            <Plus size={16} />
          </button>
        </Label>
        <Select value={categoryId} onValueChange={setCategoryId}>
          <SelectTrigger>
            <SelectValue placeholder="--Tất cả--" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">-- Tất cả --</SelectItem>
            {categories.map((c) => (
              <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>

      <div className="rounded-lg border p-4 bg-white shadow-sm">
        <Label className="mb-1 block">Tìm kiếm</Label>
        <Input placeholder="Tên hàng hóa, mã hàng..." value={q} onChange={(e) => setQ(e.target.value)} />
      </div>

      <div className="rounded-lg border p-4 bg-white shadow-sm">
        <Label className="mb-2 block">Trạng thái</Label>
        <RadioGroup
          className="flex flex-col gap-2"
          value={status}
          onValueChange={(v) => setStatus(v as any)}
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem id="st1" value="active" />
            <Label htmlFor="st1">Đang kinh doanh</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem id="st2" value="inactive" />
            <Label htmlFor="st2">Ngừng kinh doanh</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem id="st3" value="all" />
            <Label htmlFor="st3">Tất cả</Label>
          </div>
        </RadioGroup>
      </div>

      <div className="rounded-lg border p-4 bg-white shadow-sm">
        <Label className="mb-1 block">Số bản ghi</Label>
        <div className="text-sm text-slate-600">{total}</div>
      </div>
    </div>
  );
}
