  ...
  const lines: ReceiptLine[] = useMemo(() => {
    ...
    if (invItems.length) {
      return invItems.map((it: any) => {
        ...
        // origin price: prefer catalog price (niêm yết), fallback to BE fields
        const origin = Number(
          catalogItem?.price ?? it.menuItem?.price ?? it.price ?? 0
        );

        // price: prefer BE locked price; if not present, prefer catalog's priceAfterDiscount
        const price = Number(
          it.price ?? (catalogItem as any)?.priceAfterDiscount ?? origin
        );
        ...
        return {
          ...
          discountAmount,
          badge:
            discountAmount > 0 ? `-${discountAmount.toLocaleString()}đ` : null,
        } as ReceiptLine;
      });
    }
